...
export function InterviewQuestionForm({ item = {}, specialties }: { item?: QuestionValue; specialties: string[] }) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    event.preventDefault(); setSaving(true); setError(""); const body = { ...Object.fromEntries(new FormData(event.currentTarget)), answer };
  ...
  return <form className="termForm interviewQuestionForm" onSubmit={submit}>
    ...
    <label>Câu trả lời gợi ý *</label><CompactRichEditor value={item.answer} onChange={setAnswer} placeholder="Nhập câu trả lời đầy đủ, dễ ôn tập..." />
